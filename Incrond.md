...
The global config file is `/etc/incron.conf`. Now make sure you start the service (might be a good idea to `chkconfig` this too):
...
You can find an explanation of each in `/usr/include/linux/inotify.h`. For example:
...
Start editing your incrontab with `incrontab *-e*`. Here are some examples of traps
