...
Edit `/etc/httpd/conf.d/mod_gnutls.conf` and uncomment (or add) the following:
...
I create a configuration file for each in `/etc/httpd/conf.d/`. Let's start with `test.example.com`. Here's a skeleton:
