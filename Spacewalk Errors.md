...
Run this [as a cron job every night](https://www.redhat.com/archives/spacewalk-list/2010-September/msg00078.html) on **the server**:
...
All three commands should return the same thing (host.domain.tld)
    ...
    openssl x509 -text -in cat /etc/rhn/rhn.conf | grep osa_ssl_cert | sed 's/.* =//' | grep '^ :space:*Subject:.*CN=' | sed 's/.*CN=//'
    openssl x509 -text -in grep '^ :space:*<id\ .*pemfile="' /etc/jabberd/c2s.xml | sed 's/.*pemfile="\\([^"]*\\) .*/\1/' | grep '^ :space:*Subject:.*CN=' | sed 's/.*CN=\\(.*\\)[\/$].*/\1/'
    grep '^:space:*<id\ .*pemfile="' /etc/jabberd/c2s.xml | sed 's/.*>\\(.*\\).*<.*/\1/'
...
  service osad stop   rm /etc/sysconfig/rhn/osad-auth.conf   service osad start
