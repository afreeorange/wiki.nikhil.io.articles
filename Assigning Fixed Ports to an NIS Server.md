...
The motivation is to create better firewalls. You can find out what ports your server's listening on by issuing `rpcinfo`.
...
You can choose a port that's on that list or come up with your own. I'm going to use:
...
Restart the `ypserv` and `ypxfrd` services. `rpcinfo` should now show the fixed ports:
