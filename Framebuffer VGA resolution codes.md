Add a `vga=` and then the value below as a kernel parameter (at the end of `kernel`) in `/boot/grub/menu.lst`. These values have been variously gathered from across the interwebs.
