...
I edit `/etc/mysql-zrm/mysql-zrm.conf` to change the default backup destination.
...
There are [plenty of other options available](http://wiki.zmanda.com/index.php/How_do_I_configure_MySQL_ZRM).
...
I'm going to try encrypted backups. You can create a passphrase file called `.passphrase` file in the backupset directory or define your own name for it. I'm trying the latter option.
...
Make sure that your backup user [has the appropriate privileges](http://wiki.zmanda.com/index.php/Do_I_need_to_make_changes_to_MySQL_database_configuration%3F#MySQL_backup_user).
...
ZRM will use the "mysql" user to connect. Therefore, we need to set up keyless SSH access. On CentOS, the mysql user's homedir is `/var/lib/mysql`. So:
...
Now place the key of the backup server in the authorized keys file and test a connection:

    ssh mysql@client.example.com

If you log in, you're good to go! You can alternately create another user if you don't want to muck with the MySQL user.
...
If all goes well, you should see a timestamped backup folder in the backup destination (`/backups/databases` in our case) with a compressed file called `backup-data`
...
There are a [a lot of other options available](http://wiki.zmanda.com/index.php/Mysql-zrm-reporter). I wanted global reports and set this in my crontab:
...
Set the retention policy for a given backup set and run this against your backup destination:
...
The MySQL user's homedir on Ubuntu is `/nonexistent`. While this makes sense, you may want to change this for keyless access.
